package com.gitant.sfdrecipeapp.bootstrap;

import com.gitant.sfdrecipeapp.domain.*;
...
public class RecipeBootstrap implements ApplicationListener<ContextRefreshedEvent> {
    ...
    private List<Recipe> getRecipes() {
        ...
        //get UOMs
        var eachUomOptional = getUnitOfMeasure("Each");
        var tableSpoonUomOptional = getUnitOfMeasure("Tablespoon");
        var teaSpoonUomOptional = getUnitOfMeasure("Teaspoon");
        var dashUomOptional = getUnitOfMeasure("Dash");
        var pintUomOptional = getUnitOfMeasure("Pint");
        var cupsUomOptional = getUnitOfMeasure("Cup");
        ...
        //get Categories
        Optional<Category> americanCategoryOptional = getCategory("American");
        Optional<Category> mexicanCategoryOptional = getCategory("Mexican");
    ...
    private Optional<Category> getCategory(String category) {
        var americanCategoryOptional = categoryRepository.findByCategoryName(category);

        if (americanCategoryOptional.isEmpty()) {
            throw new RuntimeException("Expected category " + category + " not found.");
        }
        return americanCategoryOptional;
    }

    private Optional<UnitOfMeasure> getUnitOfMeasure(String uom) {
        var eachUomOptional = unitOfMeasureRepository.findByUom(uom);

        if (eachUomOptional.isEmpty()) {
            throw new RuntimeException("Expected UOM " + uom + " not found.");
        }
        return eachUomOptional;
    }
